...
class ParticleFilter{
  public:
    ParticleFilter(int lenOfMap, int num_particles, float translation_variance, float rotation_variance, float measurement_variance, int iter);
    void move_particles(float x, float y, float theta);
    void measure();
    void resample(float maxprob);
    void print_particles();
    void estimate_position();
    void getParticleFilterPosition(int x, int y, int heading);
    struct Particle{
      public:
          float x;
          float y;
