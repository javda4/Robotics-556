...
void PrintOLED::print_int(int x) {
  char buf[6];  // Enough to hold values up to 5 digits + null terminator

  // Update the OLED screen every 100 ms.
  if ((uint16_t)(millis() - _lastUpdateTime) > 100)
  {
    _lastUpdateTime = millis();
    snprintf(buf, sizeof(buf), "%d", x);  // Convert int to string
  }

  display.gotoXY(0, 0);  // Set cursor to top-left
  display.print(buf);    // Print the integer
}
